<nav class="bg-gray-800">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 items-center justify-between">
                ...
                <div class="hidden md:block">
                    <div class="ml-10 flex items-baseline space-x-4">
                        ...
                        <a href="/about" class="<?= urlIs('/about')
                                                    ...
                                                    :  'rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white';
                                                ?>">About</a>
                        <a href="/notes" class="<?= (urlIs('/notes') || urlIs('/note') || urlIs('/notes/create') || urlIs('/note/edit'))
                                                    ? 'rounded-md bg-gray-950/50 px-3 py-2 text-sm font-medium text-white'
                                                    :  'rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white';
                                                ?>">Notes</a>
                    ...
                    <?php if (isset($_SESSION['user'])): ?>
                        ...
                        <el-dropdown class="relative ml-3">
                            <button class="relative flex max-w-xs items-center rounded-full focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500 cursor-pointer">
                                ...
                                <p class="text-white"><?php echo $_SESSION['user']['name'] ?></p>
                            </button>
            ...
            <div class="-mr-2 flex md:hidden">
                <!-- Mobile menu button -->
                <button type="button" command="--toggle" commandfor="mobile-menu" class="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-white/5 hover:text-white cursor-pointer">
    ...
    <el-disclosure id="mobile-menu" hidden class="block md:hidden">
        <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
            <!-- Current: "bg-gray-950/50 text-white", Default: "text-gray-300 hover:bg-white/5 hover:text-white" -->
            <a href="/" aria-current="page" class="<?= urlIs('/')
            ...
            <a href="/about" class="<?= urlIs('/about')
                                        ...
                                        :  'block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-white';
                                    ?>">About</a>
            <a href="/notes" class="<?= (urlIs('/notes') || urlIs('/note') || urlIs('/notes/create'))
                                        ? 'block rounded-md bg-gray-950/50 px-3 py-2 text-base font-medium text-white'
                                        :  'block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-white';
                                    ?>">Notes</a>
            <a href="/contact" class="<?= urlIs('/contact')
        ...
        <div class="border-t border-white/10 pt-4 pb-3 px-2">
            <div class="flex items-center">
                <?php if (isset($_SESSION['user'])): ?>
                    <div class="ml-3">
                        <div class="text-base/5 font-medium text-white"><?php echo $_SESSION['user']['name'] ?></div>
                        <div class="text-sm font-medium text-gray-400"><?php echo $_SESSION['user']['email'] ?></div>
                    </div>
                    <button type="button" class="relative ml-auto shrink-0 rounded-full p-1 text-gray-400 hover:text-white focus:outline-2 focus:outline-offset-2 focus:outline-indigo-500 cursor-pointer">
                        <span class="absolute -inset-1.5"></span>
                        <span class="sr-only">View notifications</span>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" data-slot="icon" aria-hidden="true" class="size-6">
                            <path d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0" stroke-linecap="round" stroke-linejoin="round" />
